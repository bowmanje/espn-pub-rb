  ...
  describe '#games' do
    ...
    context 'when the game_date is nil' do
      ...
      it 'sends a request to the ESPN games path' do
        expect_any_instance_of(EspnPub::Client).to receive(:send_request).with(path).and_return(games_response)
        subject
      end
      ...
      context 'when the request is successful' do
        it 'returns the games from the response' do
          subject.each_with_index do |game, idx|
            ...
            expect(game.home_team_id).to eq(games_payload[idx].dig('competitions', 0, 'competitors', 0, 'id'))
            expect(game.away_team_id).to eq(games_payload[idx].dig('competitions', 0, 'competitors', 1, 'id'))
